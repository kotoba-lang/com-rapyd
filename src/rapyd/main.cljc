(ns rapyd.main
  "Kotodama WASM entrypoint for the Rapyd clean-room actor (L4) — Clojure port.
  ...
  facts (`rapyd.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"rapyd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rapyd-compat" :tier tier :entities entities} 200])
